...
def main(args):
    ...
    collected_fitters = []
    for shape in ["gaussian"]:
        this_fitter = fit_without_constraints(myfitter, shape=shape)
        this_fitter.label = "without constraints"
        collected_fitters.append(this_fitter)
        #  myfitter.plot_fit()
        #  myfitter.plot_residuals()
        #  myfitter.plot_fit_with_residuals()

        this_fitter = fit_with_constraints(myfitter, shape=shape)
        this_fitter.label = "with constraints"
        collected_fitters.append(this_fitter)
    ...
    myfitter.apply_legend(axes=axes)
    plt.show()


def fit_with_constraints(myfitter, shape="gaussian"):
    # prepare fitting
    this_fitter = copy.deepcopy(myfitter)
    this_fitter.shape = shape
...
def fit_without_constraints(myfitter, shape="gaussian"):
    # prepare fitting
    this_fitter = copy.deepcopy(myfitter)
    this_fitter.shape = shape
    this_fitter.run_fit()
